...
class OX4_Api_Client extends Zend_Rest_Client 
{
    var $path_prefix = '/ox/4.0';

    public function __construct($uri, $email, $password, $consumer_key, $consumer_secret, $oauth_realm, $cookieJarFile = './OX4_Api_CookieJar.txt', $sso = array())
    {
        ...
        $result = $this->put('/session/validate');
        ...
        if ($result->isError()) {
            ...
            $oAuth = new OX4_Oauth_Consumer($config);
    ...
    {
        $current         = json_decode($this->get('/' . $entity . '/' . $id)->getBody());

        $params = array('action' => 'update');
        $requiredFields  = json_decode($this->get('/' . $entity . '/requiredFields', $params)->getBody());
        $availableFields = json_decode($this->get('/' . $entity . '/availableFields')->getBody());
        ...
        return $this->post('/' . $entity . '/' . $id, $update);
    }
}
 ...
 *
 * Note - Zend_Oauth_Consumer is extended to make use of the OX4_OAuth_Config and OX4_Oauth_Http_RequestToken classes
 ...
 */
class OX4_Oauth_Consumer extends Zend_Oauth_Consumer
{
    /**
    ...
    {
        $this->_config = new OX4_Oauth_Config;
        if ($options !== null) {
    ...
    public function getRequestToken(
        ...
        $httpMethod = null,
        OX4_Oauth_Http_RequestToken $request = null
    ) {
        if ($request === null) {
            $request = new OX4_Oauth_Http_RequestToken($this, $customServiceParameters);
    ...
    public function getAccessToken(
        ...
        if (is_null($request)) {
            $request = new OX4_Oauth_Http_AccessToken($this);
        }
...
class OX4_Oauth_Http_AccessToken extends Zend_Oauth_Http_AccessToken
{
    /**
...
class OX4_Oauth_Http_RequestToken extends Zend_Oauth_Http_RequestToken
{
    /**
     * Generate and return a HTTP Client configured for the Header Request Scheme
...
class OX4_Oauth_Config extends Zend_Oauth_Config
{
    /**
